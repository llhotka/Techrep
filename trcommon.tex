...
\def\put_insert#1{\bgroup
  \_let\_ins{#1_ins}\_let\_left{#1_left}%
  \ifdim\ht\_ins>0pt
    \global\total_ins=\ht\_ins
    \global\advance\total_ins by \dp\_ins
    \global\advance\_left by -\bigskipamount
    \global\advance\_left by -\total_ins
    \unvbox\_ins
    \bigskip
  \fi\egroup}
...
\def\pagecontents{\ifvoid\margin_ins\else % marginal info is present
  ...
  \global\total_ins=0pt
  \put_insert{fig}%
  \advance\total_ins by \ht\tab_ins
  \advance\total_ins by \dp\tab_ins
  \ifdim\total_ins>\vsize\else
    \put_insert{tab}%
  \fi
...
\def\section#1#2{\ifvmode\else\par\fi
    ...
    \ifin_rep\else
        \errmessage{Report not started yet!}\fi
...
\def\table#1{\def\cap_label{Table~#1}%
  \add_insert{tab}\smallskip}
\def\endTable{\smallskip\end_insert}
