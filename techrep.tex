...
%\def\mono_fam{Etelka Monospace Pro}
...
\def\numberedVerbatim{\par\begingroup\tt
  ...
  \everypar={\incr\verb_lno \ifproof_mode \incr\text_lno\fi
    \llap{\ifproof_mode
        \hbox{\petit$\number\text_lno.$}\hskip.7em\fi
      \hbox to\parindent{%
        \hfil{\ix_i\number\verb_lno\ \ }}}}}
...
\def\verbatim{\par\medskip\begingroup\tt\parindent=0pt
  \obeylines\uncatcode_specials\obeyspaces
  \ifproof_mode
    \everypar={\global\incr\text_lno
      \hbox to-\leftskip{\hskip-\leftskip\hss
          \hbox{\petit$\number\text_lno.$}%
          \hskip.7em}}
  \fi}
...
% Line numbering
\text_lno=0
\def\beginPar{\ifproof_mode \begingroup 
    \def\par{\ifhmode\complete_par\fi}
    \setbox\para_box=\vbox\bgroup
  \fi}

\def\complete_par{\endgraf
    \global\advance\text_lno by \prevgraf
    \count_one=\text_lno \setbox\box_one=\hbox{}
    \loop \unskip \unpenalty \setbox2=\lastbox
      \ifhbox2 \global\setbox\box_one=
        \hbox{\hbox to-\leftskip{\hskip-\leftskip\hss
                 \hbox{\petit$\number\count_one.$}%
                 \hskip.7em}%
              \box2\penalty0\unhbox\box_one}
        \decr\count_one
    \repeat
    \egroup
    \noindent\unhbox\box_one\unpenalty
    \endgraf \endgroup}
